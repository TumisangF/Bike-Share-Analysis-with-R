setwd('C:/Users/T Fokase/Desktop/Analysis using R')
# Cyclistic_Exercise_Full_Year_Analysis

### This analysis is for case study 1 from the Google Data Analytics Certificate (Cyclistic). It’s
###originally based on the case study "'Sophisticated, Clear, and Polished’: Divvy and Data
###Visualization" written by Kevin Hartman (found here:
###https://artscience.blog/home/divvy-dataviz-case-study). We will be using the Divvy dataset for the case study. 

#The purpose of this script is to consolidate downloaded Divvy data into a single
#dataframe and then conduct simple analysis to help answer the key question: “In what ways do
#members and casual riders use Divvy bikes differently?”

# # # # # # # # # # # # # # # # # # # # # # #
install.packages('tidyverse')   # tidyverse for data import and wrangling
install.packages('libridate')   # libridate for date functions
...
all_trips <- all_trips %>%
  select(-c(start_lat, start_lng, end_lat, end_lng, birthyear, gender, "05 - Member Details Member Birthday Year", "Member Gender",
            "tripduration"))
...
#=====================================
#Descriptive analysis on the ride_length (all figures in seconds)
...
all_trips_v2$day_of_week <- ordered(all_trips_v2$day_of_week, levels=c('Sunday','Monday','Tuesday','Wednesday','Thurday','Friday','Saturday'))
...
all_trips_v2 %>%
  ...
  labs(title = 'Average duration against weekdays') +
  geom_col(position = "dodge")
